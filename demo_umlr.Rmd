---
title: "Metalearner-Based Average Treatment Effect Estimation: MLR vs. UMLR"
...
```{r, echo=FALSE}
### Tuning Summary
tuning_df_s <- data.frame(
  Method = c("KRR", "KRR", "KRR-UP", "KRR-UP"),
  Arm = c("Treated", "Control", "Treated", "Control"),
  Lambda = c(tune_krr_s$lambda, tune_krr_s$lambda,
             tune_krrup_s$lambda, tune_krrup_s$lambda),
  Sigma = round(c(tune_krr_s$sigma, tune_krr_s$sigma,
                  tune_krrup_s$sigma, tune_krrup_s$sigma), 4),
  Criterion = c("RMSE", "RMSE", "|cor(resid,y)|", "|cor(resid,y)|"),
  Value = round(c(tune_krr_s$crit,   tune_krr_s$crit,
                  tune_krrup_s$crit, tune_krrup_s$crit), 4)
)
```
...
```{r, echo=FALSE}
ate_df_s <- data.frame(
  Metric = c("Estimated ATE", "ATE Bias", "RMSE"),
  True_ATE = c(round(ate_true, 4), "—", "—"),
  KRR = c(round(ate_krr_s, 4),
          round(ate_krr_s - ate_true, 4),
          round(rmse_cate_krr_s, 4)),
  KRR_UP = c(round(ate_krrup_s, 4),
             round(ate_krrup_s - ate_true, 4),
             round(rmse_cate_krrup_s, 4))
)

kable(ate_df_s,
      col.names = c("", "True ATE", "MLR", "UMLR"),
      caption = "Table 1. ATE Estimation Results — S-Learner",
      align = c("l", "r", "r", "r"))
```
...
```{r, echo=FALSE}
### Tuning Summary
tuning_df_t <- data.frame(
  Method = c("KRR", "KRR", "KRR-UP", "KRR-UP"),
  Arm = c("Treated", "Control", "Treated", "Control"),
  Lambda = c(tune_krr_t1$lambda, tune_krr_t0$lambda,
             tune_krrup_t1$lambda, tune_krrup_t0$lambda),
  Sigma = round(c(tune_krr_t1$sigma, tune_krr_t0$sigma,
                  tune_krrup_t1$sigma, tune_krrup_t0$sigma), 4),
  Criterion = c("RMSE", "RMSE", "|cor(resid,y)|", "|cor(resid,y)|"),
  Value = round(c(tune_krr_t1$crit,   tune_krr_t0$crit,
                  tune_krrup_t1$crit, tune_krrup_t0$crit), 4)
)
```
...
ate_df_t <- data.frame(
  Metric = c("Estimated ATE", "ATE Bias", "RMSE"),
  True_ATE = c(round(ate_true, 4), "—", "—"),
  KRR = c(round(ate_krr_t, 4),
          round(ate_krr_t - ate_true, 4),
          round(rmse_cate_krr_t, 4)),
  KRR_UP = c(round(ate_krrup_t, 4),
             round(ate_krrup_t - ate_true, 4),
             round(rmse_cate_krrup_t, 4))
)

kable(ate_df_t,
      col.names = c("", "True ATE", "MLR", "UMLR"),
      caption = "Table 2. ATE Estimation Results — T-Learner",
      align = c("l", "r", "r", "r"))
```

```{r, include=FALSE}
## Check Eta
eta_krr_t_11 <- coef(lm(mu1_hat_krr_t[idx1] ~ Y1[idx1]))[2]
eta_krr_t_01 <- coef(lm(mu0_hat_krr_t[idx1] ~ Y0[idx1]))[2]
eta_krr_t_00 <- coef(lm(mu0_hat_krr_t[idx0] ~ Y0[idx0]))[2]
eta_krr_t_10 <- coef(lm(mu1_hat_krr_t[idx0] ~ Y1[idx0]))[2]

eta_krrup_t_11 <- coef(lm(mu1_hat_krrup_t[idx1] ~ Y1[idx1]))[2]
eta_krrup_t_01 <- coef(lm(mu0_hat_krrup_t[idx1] ~ Y0[idx1]))[2]
eta_krrup_t_00 <- coef(lm(mu0_hat_krrup_t[idx0] ~ Y0[idx0]))[2]
eta_krrup_t_10 <- coef(lm(mu1_hat_krrup_t[idx0] ~ Y1[idx0]))[2]
```

```{r, echo=FALSE, include=FALSE}
### Eta Summary
eta_df_t <- data.frame(
  Regression = c("eta(Y1_hat ~ Y1, T=1)  factual",
                 "eta(Y0_hat ~ Y0, T=1)  counterfactual",
                 "eta(Y0_hat ~ Y0, T=0)  factual",
                 "eta(Y1_hat ~ Y1, T=0)  counterfactual"),
  KRR = round(c(eta_krr_t_11, eta_krr_t_01,
                eta_krr_t_00, eta_krr_t_10), 4),
  KRR_UP = round(c(eta_krrup_t_11, eta_krrup_t_01,
                   eta_krrup_t_00, eta_krrup_t_10), 4)
)

kable(eta_df_t, col.names = c("Regression", "KRR", "KRR-UP"),
      caption = "Eta Summary")
```
...
ate_df_x <- data.frame(
  Metric = c("Estimated ATE", "ATE Bias", "RMSE"),
  True_ATE = c(round(ate_true, 4), "—", "—"),
  KRR = c(round(ate_krr_x, 4),
          round(ate_krr_x - ate_true, 4),
          round(rmse_cate_krr_x, 4)),
  KRR_UP = c(round(ate_krrup_x, 4),
             round(ate_krrup_x - ate_true, 4),
             round(rmse_cate_krrup_x, 4))
)

kable(ate_df_x,
      col.names = c("", "True ATE", "MLR", "UMLR"),
      caption = "Table 3. ATE Estimation Results — X-Learner",
      align = c("l", "r", "r", "r"))
```
...
```{r, echo=FALSE}
ate_df_dr <- data.frame(
  Metric = c("Estimated ATE", "ATE Bias", "RMSE"),
  True_ATE = c(round(ate_true, 4), "—", "—"),
  KRR = c(round(ate_krr_aipw, 4),
          round(ate_krr_aipw - ate_true, 4),
          round(rmse_cate_krr_aipw, 4)),
  KRR_UP = c(round(ate_krrup_aipw, 4),
             round(ate_krrup_aipw - ate_true, 4),
             round(rmse_cate_krrup_aipw, 4)),
  DML = c(round(ate_dml, 4),
          round(ate_dml - ate_true, 4),
          "—")
)

kable(ate_df_dr,
      col.names = c("", "True ATE", "MLR", "UMLR", "Double ML"),
      caption = "Table 4. ATE Estimation Results — Doubly Robust",
      align = c("l", "r", "r", "r", "r"))
```
